...
class wfp_Permissions extends XoopsGroupPermForm {
     ...
     * @return
     */
    function setPermissions( $table = '', $perm_name = '', $perm_descript = '', $mod_id ) {
        if ( !empty( $table ) ) {
            $this->db = &XoopsDatabaseFactory::getDatabaseConnection();
            $this->db_table = $this->db->prefix( $table );
        }
        $this->_mod_id = intval( $mod_id );
     ...
     * @return
     */
    function save( $groups = array(), $item_id = 0 ) {
     ...
     * @return
     */
    function getAdmin( $item_id, $isNew = null ) {
        $item_id = intval( $item_id );
        $gperm_handler = &wfp_gethandler( 'groupperm' );
